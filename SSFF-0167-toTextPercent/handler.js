exports.handler = async (event) => {
  let porcentaje = event.porcentaje;
  let porcentajeTextoArray = [];

  const pArray = porcentaje.split('.');
  pArray.forEach(e =>
    porcentajeTextoArray.push(NumeroALetras(e))
  );
  let result = `${porcentajeTextoArray[0]}-PUNTO-${porcentajeTextoArray[1]}-PORCIENTO`
  if (porcentaje === undefined || porcentaje === '') result = '';

  const response = {
    statusCode: 200,
    body: {
      porcentaje: (event.porcentaje),
      texto: result.split('--').join('-')
    },
  };
  return response;
};
