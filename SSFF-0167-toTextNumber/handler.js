exports.handler = async (event) => {
  ...
  let numeroTextoArray = ``;



  const nArray = numero.split('');
  nArray.forEach(e =>
    numeroTextoArray = numeroTextoArray.concat(' ' + NumeroALetras(e))
  );

  numeroTextoArray = numeroTextoArray.trim().split(' ').join('-');
  if (numero == undefined || numero == '') numeroTextoArray = '';
  const response = {
    statusCode: 200,
    body: {
      numero: event.numero,
      texto: `${numeroTextoArray}`
    },
  };
  return response;
};
