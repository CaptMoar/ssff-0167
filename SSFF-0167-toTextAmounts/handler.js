exports.handler = async (event) => {
  let montos = (JSON.stringify(event.monto).replace('$', ''));
  montos = montos.split(',').join('')
  console.log(montos)
  let result = (NumeroALetras(montos).trim()).split(' ').join('-');
  if (montos == undefined || montos == '') result = '';
  const response = {
    statusCode: 200,
    body: {
      monto: event.monto,
      texto: (result).split('--').join('-') + '-PESOS'
    },
  };
  return response;
};
