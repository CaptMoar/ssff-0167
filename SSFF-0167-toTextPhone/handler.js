exports.handler = async (event) => {
  let telefono = event.telefono;
  let telefonoTextoArray = [];

  const fArray = telefono.split('');
  fArray.forEach(e =>
    telefonoTextoArray.push(NumeroALetras(e))
  );

  if (telefono == undefined || telefono == '') telefono = '';

  const response = {
    statusCode: 200,
    body: {
      telefono,
      texto: telefonoTextoArray.join('-')
    },
  };
  return response;
};
